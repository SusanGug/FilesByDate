...
class TestFileManagementLogic(unittest.TestCase):
    ...
    def test_format_date_all_formats(self):
        ...
        for format_type, expected in test_cases:
            ...
            self.assertEqual(
                result,
                expected,
                f"Format {format_type} should return {expected}")

    def test_read_file_date_properties(self):
        """Test date reading from files."""
        file_name = self.test_files[0]
        date_str = self.file_management_logic.read_file_date_properties(
            file_name)
    ...
    def test_copy_operation_success(self):
        ...
        for file_name in self.test_files:
            ...
            self.assertTrue(
                ...
                f"Source file {file_name} should still exist")
        ...
        if self.file_management_logic.last_operation:  # Check if not None
            self.assertEqual(
                self.file_management_logic.last_operation['type'], 'copy')
    ...
    def test_move_operation_success(self):
        ...
        for file_name in self.test_files:
            ...
            self.assertFalse(
                os.path.exists(source_path),
                f"Source file {file_name} should be moved")

        # Verify operation tracking
        self.assertIsNotNone(self.file_management_logic.last_operation)
        if self.file_management_logic.last_operation:  # Check if not None
            self.assertEqual(
                self.file_management_logic.last_operation['type'], 'move')
    ...
    def test_folder_creation(self):
        ...
        self.assertTrue(
            len(dest_contents) > 0,
            "Date folders should be created")
        ...
        for folder_name in dest_contents:
            ...
            self.assertTrue(
                ...
                f"{folder_name} should be a directory")
    ...
    def test_undo_move_operation(self):
        ...
        self.assertEqual(
            len(source_files),
            0,
            "Source folder should be empty after move")
        ...
        self.assertEqual(
            set(restored_files), set(
                self.test_files), "All files should be restored to source")
    ...
    def test_undo_without_operation(self):
        ...
        self.assertFalse(
            undo_success,
            "Undo should fail when no operation was performed")
    ...
    def test_operation_history_limit(self):
        ...
        self.assertLessEqual(len(logic.operation_history), 10,
                             "Operation history should be limited to 10 entries")
    ...
    def test_preview_organization(self):
        ...
        self.assertTrue(
            ...
            "Preview should show file organization")

        # Verify all test files are included in preview
        total_files_in_preview = sum(len(files)
                                     for files in preview_result.values())
