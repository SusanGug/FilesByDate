...
class FileChooserApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        process_steps = tk.Label(
            process_frame, text="1. Select a source folder with files to organize\n"
            ...
            "5. Use Undo if you need to reverse the operation", font=(
                'Arial', 9), bg='#e8f4fd', fg='#34495e', justify=tk.LEFT)
        process_steps.pack(anchor=tk.W, padx=15, pady=(0, 10))

        # Settings Frame
        settings_frame = tk.LabelFrame(
            main_frame, text="⚙️ Settings", font=(
                'Arial', 12, 'bold'), bg='#f0f0f0', fg='#2c3e50')
        ...
        self.source_entry = tk.Entry(
            source_input_frame, textvariable=self.folder_organize_path, font=(
                'Arial', 10), bg='white', relief=tk.SOLID, bd=1)
        ...
        self.dest_entry = tk.Entry(
            dest_input_frame,
            textvariable=self.project_folder_path,
            font=(
                'Arial',
                10),
            bg='white',
        ...
        self.info_frame = tk.LabelFrame(
            main_frame, text="📊 File Information", font=(
                'Arial', 12, 'bold'), bg='#f0f0f0', fg='#2c3e50')
        self.info_frame.pack(fill=tk.X, pady=(0, 20))

        # File info explanation
        info_explanation = tk.Label(
            self.info_frame,
        ...
        self.file_count_label = tk.Label(
            self.info_frame, text="No source folder selected", font=(
                'Arial', 10), bg='#f0f0f0', fg='#7f8c8d')
        self.file_count_label.pack(pady=10)

        # Actions Frame
        actions_frame = tk.LabelFrame(
            main_frame, text="🚀 Actions", font=(
                'Arial', 12, 'bold'), bg='#f0f0f0', fg='#2c3e50')
        actions_frame.pack(fill=tk.X, pady=(0, 20))

        # Actions explanation
        actions_explanation = tk.Label(
            actions_frame, text="Choose how you want to organize your files:", font=(
                'Arial', 9), bg='#f0f0f0', fg='#7f8c8d', wraplength=500)
        ...
        self.operations_frame = tk.LabelFrame(
            main_frame, text="Recent Operations", font=(
                'Arial', 12, 'bold'), bg='#f0f0f0', fg='#2c3e50')
        self.operations_frame.pack(fill=tk.BOTH, expand=True)

        # Operations explanation
        operations_explanation = tk.Label(
        ...
        operations_scrollbar = tk.Scrollbar(
            operations_text_frame,
            orient=tk.VERTICAL,
    ...
    def update_button_states(self):
        ...
        has_operation = hasattr(
            self, 'file_management_logic') and self.file_management_logic

        # Enable/disable action buttons
        self.copy_button.config(state=tk.NORMAL if (
            has_source and has_dest) else tk.DISABLED)
        self.move_button.config(state=tk.NORMAL if (
            has_source and has_dest) else tk.DISABLED)
        self.undo_button.config(
            state=tk.NORMAL if has_operation else tk.DISABLED)
    ...
    def update_file_info(self):
        ...
        if source_path and os.path.exists(source_path):
            ...
            self.file_count_label.config(
                text="No source folder selected", fg='#7f8c8d')
    ...
    def undo_action(self):
        if hasattr(
                self,
                'file_management_logic') and self.file_management_logic:
            ...
            if success:
                ...
                messagebox.showwarning(
                    "Warning",
                    "No operation to undo or undo operation failed. Check console for details.")
                self.log_operation("Undo operation failed")
        else:
            messagebox.showwarning(
                "Warning", "No file operation has been performed yet.")
    ...
    def _perform_operation(self, operation_type, status_message):
        ...
        try:
            self.file_management_logic = FileManagementLogic(
                target_folder, project_folder, format_type)
            ...
            if result:
                ...
                if errors:
                    messagebox.showwarning(
                        "Warning",
                        f"Some files could not be {operation_type}d. Check console for details.")
                    self.log_operation(
                        f"⚠️ {operation_type} completed with {
                            len(errors)} errors")
                else:
                    messagebox.showinfo(
                        "Success", f"Successfully {operation_type}d {
                            len(processed_files)} files to their respective date folders.")
                    self.log_operation(
                        f"✅ {operation_type} operation completed: {
                            len(processed_files)} files processed")

                # Update button states
                self.update_button_states()
            else:
                messagebox.showerror(
                    "Error", f"{operation_type} operation failed!")
